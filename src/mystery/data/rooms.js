...
/** clueId → evidence image key, for clue-granting hotspots that carry art.
 *  Lets the notebook show a thumbnail next to each logged clue. */
export const evidenceImageByClueId = Object.fromEntries(
  rooms.flatMap((r) =>
    (r.examineTargets ?? [])
      .filter((t) => t.clueId && t.image)
      .map((t) => [t.clueId, t.image]),
  ),
);
